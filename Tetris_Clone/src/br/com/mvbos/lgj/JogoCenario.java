...
public class JogoCenario extends CenarioPadrao {
	...
	private String nome;
	private boolean inserirNome = true;
	private boolean montagemRanking = true;
	private int pyRanking = 210;
	...
	public void carregar() {
		...
		try {
			as = AudioSystem.getAudioInputStream(new File("som\\la_ele.wav"));
			clipAdicionarPeca = AudioSystem.getClip();
			clipAdicionarPeca.open(as);

			as = AudioSystem.getAudioInputStream(new File("som\\quebra_de_blocos.wav"));
			clipMarcarLinha = AudioSystem.getClip();
			clipMarcarLinha.open(as);

			seqSomDeFundo = MidiSystem.getSequencer();
			seqSomDeFundo.setSequence(MidiSystem.getSequence(new File("som\\som_de_fundo.mid")));
			seqSomDeFundo.open();
	...
	public void atualizar() {
		...
		if (Jogo.controleTecla[Jogo.Tecla.GI.ordinal()])
		{
		...
		if(seqSomDeFundo != null){
			//Se apertar S
			if(Jogo.controleTecla[Jogo.Tecla.BD.ordinal()]){
				if(seqSomDeFundo.isRunning()){
					seqSomDeFundo.stop();
				} else{
					seqSomDeFundo.start();
				}
	...
	private boolean marcarLinha() {
		...
		pontos += multPontos * multPontos*10;
	...
	public void desenhar(Graphics2D g) {
		...
		texto.desenha(g, "Level " + nivel + " - " + linhasFeistas, largura / 2 - 60, 20);
		texto.desenha(g, "Pts: " + String.valueOf(pontos), largura - 150, 20);

		if (estado != Estado.JOGANDO) {
			if(this.inserirNome){
				this.nome = JOptionPane.showInputDialog("Nome:");
				this.inserirNome = false;
			}
			g.setColor(Color.BLACK);
			g.fillRect(0,0,500,672);

			texto.setCor(Color.RED);
			if (estado == Estado.GANHOU) {
				texto.desenha(g, "WINNER!", 180, 130);
			} else {
				texto.desenha(g, "GAME OVER!", 180, 130);
			}

			g.setColor(Color.WHITE);
			g.drawLine(150, 150,320,150);

			texto.setCor(Color.WHITE);
			texto.desenha(g,"RANKING",193,175);

			Ranking ranking = new Ranking();

			ranking.leArquivo();
			if(montagemRanking) {
				ranking.adiciona(nome, pontos);
				ranking.gravar();
				montagemRanking = false;
			}
			ranking.ordenar();
			ranking.imprimir(g, pyRanking);

			if(seqSomDeFundo != null && seqSomDeFundo.isRunning()){
				seqSomDeFundo.stop();
			}
		}
	}
